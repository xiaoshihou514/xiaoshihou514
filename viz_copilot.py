...
for path in glob.glob("recent_copilot/*.json"):
    with open(path) as f:
        data = json.load(f)
...
if not lang_totals:
    raise SystemExit("No copilot data found")
...
# Write SVG
with open("copilot.svg", "w") as f:
    f.write("\n".join(svg_lines))

print(f"Copilot LOC total (excluding skipped languages): {total_lines}")
print("SVG generated: copilot.svg")
